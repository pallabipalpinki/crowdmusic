...
public function category(){
     if($this->session->userdata('SESSION_ADMIN_ID')){
          $this->data['page']='categorylist';
          $this->data['module']  = 'music';
          $this->data['list'] = $this->admin_panel_model->get_category();
          $this->load->vars($this->data);
          $this->load->view($this->data['theme'].'/template');
        }
     else{
...
public function category_add(){
 ...
 public function save_category(){
       if($this->input->server('REQUEST_METHOD')=='POST'){
         $this->form_validation->set_rules('name', 'Genres Name','trim|required|is_unique[content_speciality.spec_name]');
         $this->form_validation->set_rules('categoryshown', 'Home page','trim|required');
        // $this->form_validation->set_rules('about', 'About Category','trim|required');
              if ($this->form_validation->run() === FALSE){
                  $this->data['module']  = 'music';
                  $this->data['page']='add_category';
            ...
            $query=$this->db->query("SELECT spec_serial FROM content_speciality ORDER BY spec_id DESC LIMIT 1");
              ...
              $lastdata= $query->row();
                ...
                $added=$this->admin_panel_model->add_category($data_to_store);
                ...
                if($added){
                    ...
                    if(isset($_FILES['category_image']) && $_FILES['category_image']['name']!=''){
                        ...
                        if ( ! $this->upload->do_upload('category_image')) {
                          ...
                          //$this->resizeImage($path,$path,'366','365');
                          $this->admin_panel_model->update_category(array('spec_img_path'=>$disk_path,'spec_img'=>$relative_path),array('spec_id'=>$added));
                        }
                    }
                  ...
                  $this->session->set_flashdata('fail','There was an system error.Try after sometime.');
                      redirect('admin-category');
...
public function category_edit($cid){
        if($this->session->userdata('SESSION_ADMIN_ID')){
            ...
            $this->data['category_id'] = $cid;
            $this->data['categorydtl'] = $this->admin_panel_model->category_dtl_db($cid);
...
public function update_category(){
        if($this->session->userdata('SESSION_ADMIN_ID')){
         // print_r($this->input->post());die;
           ...
           $this->form_validation->set_rules('name', 'Category Name','trim|required|is_unique[content_speciality.spec_name]');
            //$this->form_validation->set_rules('about', 'About Category','trim|required');
            ...
            // $this->form_validation->set_rules('about', 'About Category','trim|required');
                ...
                // if($duplicate_data->counted==0){
                    ...
                    $data_to_store=array(
                     ...
                     //print_r($array); die;
                 // $genres_update=$this->admin_panel_model->update_genres_db($cid,$array);
                  $updated=$this->admin_panel_model->update_category($data_to_store,array('spec_id'=>$cid));
                    ...
                    if($updated){
                        ...
                        if(isset($_FILES['category_image']) && $_FILES['category_image']['name']!=''){
                            ...
                            if ( ! $this->upload->do_upload('category_image')) {
                              ...
                              //$this->resizeImage($path,$path,'366','365');
                              $this->admin_panel_model->update_category(array('spec_img_path'=>$disk_path,'spec_img'=>$relative_path),array('spec_id'=>$cid));
                            }
                        }
...
public function category_delete($c_id){
    //echo "hi"; die;
      $cid=$this->input->post('rmvfile');
      // $usersdata = array(
      //     'status' => 0
      // );
      $this->db->where('spec_id', $cid);
      $query = $this->db->delete('content_speciality');

      $this->db->where('slug_type_id', $cid);
      $query = $this->db->delete('slugs');
