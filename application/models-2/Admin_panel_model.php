...
public function get_category(){
    $query = $this->db->query(" SELECT * FROM `content_speciality`");
   // print_r($query->last_query()); die;
    $result = $query->result_array();

    return $result;
}
public function category_dtl_db($id){
            $query = $this->db->where('spec_id',$id);
...
public function get_duplicate_category($param=array(),$param_or=array(),$group_in=FALSE,$field=null,$return_query=FALSE){
    if($field==null){
  ...
  public function add_category($data,$batch=FALSE,$return_query=FALSE){
    if(!empty($data)){
      if($return_query==FALSE){
        if($batch==FALSE){
          $this->db->insert('content_speciality',$data);
          return $this->db->insert_id();
        }else{
          return $this->db->insert_batch('content_speciality',$data);
        } 
      }else{
        return $this->db->set($data)->get_compiled_insert('content_speciality');
      }
    }else{
...
public function update_category($data=array(),$param=array(),$batch=FALSE,$return_query=FALSE){
    if(!empty($data) && !empty($param)){
      if($return_query==FALSE){
        if($batch==FALSE){
          return $this->db->update('content_speciality',$data,$param);
        }else{
          return $this->db->update_batch('content_speciality',$data,$param);
        }
      }else{
        if($batch==FALSE){
          $this->db->update('content_speciality',$data,$param);
        }else{
          $this->db->update_batch('content_speciality',$data,$param);
        }
        return $this->db->last_query();
      }
    }else{
