jQuery(function($) {
  "use strict";
						...
						.then((value) => {
						  switch (value) {
						    ...
						    case "catch":
						      ...
						      $('#contentsModalTracksId').find('#content_album_id').val(f.uploaded_album);
	...
	$('#form_album_only_tracks').validate({
		...
		submitHandler:function(){
	      ...
	      $.ajax({
	          ...
	          target: '.preview',
	          beforeSend:function(){
	            $('#upload_album_only_track').html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>').prop('disabled',true);
	          },
	          success:function(f){
	            if(f.success){
	          ...
	          complete:function(status,xhr){
	            ...
	            $('#upload_album_only_track').html('Upload Track').attr('disabled',false);
	...
	$('body').on('click','.btn_delete',function(){
		...
		if(content_type=='1'){
			var msg='Once deleted, you will not be able to recover the album & tracks';
		}else if(content_type=='2'){
	...
	$('body').on('click','.btn_upload_track',function(){
		$('#form_album_only_tracks').find('#content_album_id').val($(this).attr('data-album'));
		$('#contentsModalTracksId').modal('show');
	});
	...
	$('body').on('click','.btn_album_edit',function(){
		...
		$('#contentsModalId').find('#content_title').val($(this).attr('data-album_name'));
		$('#contentsModalId').find('#content_desc').val($(this).attr('data-album_about'));
	...
	$('body').on('click','.btn_content_edit',function(){
		...
		$('#contentsModalId').find('#content_title').val($(this).attr('data-content_name'));
		$('#contentsModalId').find('#content_desc').val($(this).attr('data-content_about'));
		$('#contentsModalId').find('.modal-header h2').html('Update Track');
		$('#contentsModalId').modal('show');
	});

	$('body').on('click','.btn_view_tracks',function(){
		
		var html='';
		$.ajax({
			type:'POST',
			url:base_url+'load_contents',
			data:{_content_type:'2',album_id:$(this).attr('data-album')},
			beforeSend:function(){
			...
			success:function(d){
				if(d!=''){
					...
					var i=0;
					$.each(d,function(k,v){
						var content_id=v.content_id;
						
						html+='<div class="col-6 col-md-4 col-lg-3">';
						    html+='<div class="top-item">';
						       html+='<img src="'+v.content_image+'" alt="">';

						       	html+='<div id="title'+i+'" class="caption">';
									    html+='<span id="track'+i+'" class="track" style="display: none;"></span>';
									    html+='<div id="timer'+i+'" class="timer">0:00</div>';
									    html+='<div id="duration'+i+'" class="duration">0:00</div>';
									  html+='</div>';

									  
									  html+='<div class="controlsOuter">';
									    html+='<div class="controlsInner">';
									      html+='<div id="loading'+i+'"></div>';
									      html+='<div class="btn playBtn" id="playBtn'+i+'"></div>';
									      html+='<div class="btn pauseBtn" id="pauseBtn'+i+'"></div>';
									      html+='<div class="btn prevBtn" id="prevBtn'+i+'" style="display: none;"></div>';
									      html+='<div class="btn nextBtn" id="nextBtn'+i+'" style="display: none;"></div>';
									    html+='</div>';
									    html+='<div class="btn playlistBtn" id="playlistBtn'+i+'"></div>';
									    html+='<div class="btn volumeBtn" id="volumeBtn'+i+'"></div>';
									  html+='</div>';

									  
									  html+='<div class="waveform" id="waveform'+i+'"></div>';
									  html+='<div class="bar" id="bar'+i+'"></div>';
									  html+='<div class="progress" id="progress'+i+'"></div>';

									 
									  html+='<div class="playlist" id="playlist'+i+'">';
									    html+='<div class="list" id="list'+i+'"></div>';
									  html+='</div>';

									 
									  html+='<div class="volume fadeout" id="volume'+i+'">';
									    html+='<div class="barFull bar" id="barFull'+i+'"></div>';
									    html+='<div class="barEmpty bar" id="barEmpty'+i+'"></div>';
									    html+='<div class="sliderBtn" id="sliderBtn'+i+'"></div>';
									  html+='</div>';
						 ...
						 var_arr[]=['track'+i, 'timer'+i, 'duration'+i, 'playBtn'+i, 'pauseBtn'+i, 'prevBtn'+i, 'nextBtn'+i, 'playlistBtn'+i, 'volumeBtn'+i, 'progress'+i, 'bar'+i, 'wave'+i, 'loading'+i, 'playlist'+i, 'list'+i, 'volume'+i, 'barEmpty'+i, 'barFull'+i, 'sliderBtn'+i];
				...
				html+='<script src="https://localhost/crowdmusiq/assets/js/_player.js"></script>';

				$('#contentsTracksModalId').find('#album_tracks').html(html);

			}
		});


		$('#contentsTracksModalId').modal('show');
	});
	...
	function loadContents(content_type){
		...
		$.ajax({
			type:'POST',
			url:base_url+'load_contents',
			data:{_content_type:content_type,album_id:null},
			beforeSend:function(){
			...
			success:function(d){
				if(d!=''){
					if(content_type=='2'){
						$.each(d,function(k,v){
							html+='<section class="col-lg-4 col-md-4 col-sm-6 m_top_30" id="contentdiv_'+v.content_id+'"> <img src="'+v.content_image+'" class="content_img" loading="lazy"/><div class="btn-group btn_group"><button type="button" class="btn btn-xs btn-success btn_content_edit" data-content="'+v.content_id+'" data-content_name="'+v.content_track_name+'"  data-content_about="'+v.content_about+'"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</button><button class="btn btn-xs btn-danger btn_delete" data-content_type="2" data-content_id="'+v.content_id+'"><i class="fa fa-trash"></i></button> </section></div>';
						});
					}else if(content_type=='1'){
						$.each(d,function(k,v){
							html+='<section class="col-lg-4 col-md-4 col-sm-6 m_top_30" id="albumdiv_'+v.album_id+'"> <img src="'+v.album_image_file+'" class="content_img" loading="lazy"/>';
							html+='<div class="btn-group btn_group">';
							html+='<button type="button" class="btn btn-xs btn-primary btn_view_tracks" data-album="'+v.album_id+'" data-toggle="tooltip" data-placement="top" title="Upload Tracks">Tracks</button>';
							html+='<button type="button" class="btn btn-xs btn-warning btn_upload_track" data-album="'+v.album_id+'" data-toggle="tooltip" data-placement="top" title="Upload Tracks"><i class="fa fa-upload" aria-hidden="true"></i></button>';
							  html+='<button type="button" class="btn btn-xs btn-success btn_album_edit" data-album="'+v.album_id+'" data-album_name="'+v.album_name+'"  data-album_about="'+v.album_about+'"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</button>';						  
							  html+='<button class="btn btn-xs btn-danger btn_delete" data-content_type="1" data-content_id="'+v.album_id+'"><i class="fa fa-trash"  aria-hidden="true"></i></button>';
							html+='</div>';
